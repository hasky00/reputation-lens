...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/x-health") {
    await handleXHealth(res);
    return;
  }
...
server.listen(PORT, () => {
  console.log(`Reputation Lens running at http://localhost:${PORT}`);
  console.log(`X bearer token: ${reputation.tokenFingerprint(process.env.X_BEARER_TOKEN)}`);
});
...
async function handleScan(url, res) {
  ...
  if (!handle) {
    sendJson(res, 400, { error: "Enter an X handle to scan." });
    return;
  }

  if (!bearer) {
    sendJson(res, 503, { error: "Missing X_BEARER_TOKEN in .env" });
    return;
  }

  try {
    sendJson(res, 200, {
      profile: await reputation.scanHandle(handle, bearer),
    });
  } catch (error) {
...
async function handleXHealth(res) {
  const bearer = process.env.X_BEARER_TOKEN;
  ...
  try {
    const usage = await reputation.getUsage(bearer);
    sendJson(res, 200, { ok: true, token: reputation.tokenFingerprint(bearer), usage });
  } catch (error) {
    sendJson(res, error.status || 500, {
      ok: false,
      token: reputation.tokenFingerprint(bearer),
...
async function xFetch(resource, bearer, label) {
  const response = await fetch(resource, {
    headers: {
      ...
      "User-Agent": "reputation-lens-local-mvp",
    },
  });
...
function buildProfile(user, tweets) {
  ...
  if (linkRatio > 0.65) {
    score += 16;
    risks.push(["Heavy link pattern", `${percent(linkRatio)} of recent posts include links.`, "danger"]);
